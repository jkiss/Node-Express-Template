...
function requestToken() {
    return new Promise((resolve, reject) => {
        ...
        request({
            method: 'PUT',
            uri: 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' + wx_appid + '&secret=' + wx_appsecret
        }, (error, response, body) => {
            if (!error && response.statusCode == 200) {
                ...
                if (!token_data['errcode']) {
                    ...
                    redis.set(wx_token_key, access_token, expires_in - config.expireOffset, (err, reply) => {
                        if (err) {
        ...
        request({
            ...
            if (!error && response.statusCode == 200) {
                ...
                if (ticket_data['errcode'] == 0) {
                    ...
                    redis.set(wx_ticket_key, ticket, expires_in - config.expireOffset, (err, reply) => {
                        if (err) {
                    ...
                    logger.error('Request sucess, but get ticket failed: ' + ticket_data['errmsg']);
                    reject('Request sucess, but get ticket failed: ' + ticket_data['errmsg']);
                }
            } else {
...
function wxJssdkAuth(url, cb) {
    ...
    if (!url) {
        ...
        getToken()
            ...
            .then((ticket) => {
                var signature = getWxSignature(originUrl, ticket, timestamp, noncestr);
