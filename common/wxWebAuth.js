            ...
            (error, response, body) => {
                if (!error && response.statusCode == 200) {
                    ...
                    if (!token_data['errcode']) {
                        console.info('Get wxWebAuth Token from Internet...')
                        resolve(token_data)
                    } else {
                        console.error(
                    ...
                    console.error('wxWebAuth Token Request failed: ' + error)
            ...
            (error, response, body) => {
                if (!error && response.statusCode == 200) {
                    ...
                    if (!user_data['errcode']) {
                        console.info('Get wxWebAuth User data from Internet...')
                        resolve(user_data)
                    } else {
                        console.error(
                            'Request sucess, but get user data failed: ' +
                                user_data['errmsg']
                        )
                        reject(
                            'Request sucess, but get user data failed: ' +
                    ...
                    console.error(
                        'wxWebAuth User data Request failed: ' + error
                    )
